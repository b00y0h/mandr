...
class mysiteSkinGenerator extends mysiteOptionGenerator {
	...
	function __construct() {
		...
		$this->styles_dir();
		$this->activate_skin( 'steelblue.css', $suppress_msg = true, $init = true );
	...
	function json_response( $args = array() ) {
		...
		elseif( $type == 'upload_error' )
			return sprintf( __( 'Error: The skin %1$s could not be uploaded, unzip it manually and upload it to the following directory on your server: %2$s', MYSITE_ADMIN_TEXTDOMAIN ), $name, str_replace( $_SERVER['DOCUMENT_ROOT'] . '/', '', $this->stylesdir ) );
		...
		if( is_multisite() || $export == true ) {
			...
			if( strpos( $image, 'http' ) === false ) {
				...
				if( $export ) {
					...
					return 'url(' . str_replace(str_replace( $_SERVER['DOCUMENT_ROOT'], '', THEME_DIR . '/styles//' ), '/', $path ) . ')';
				}
	...
	function file_write( $name, $overwrite = false ) {
		...
		if( !empty( $is_mu ) && !@is_file( $new_stylesheet ) )
	...
	function activate_skin( $name, $suppress_msg = true, $init = false ) {
		...
		foreach( $this->parse_options() as $key => $value ) {
			if( !empty( $value['name'] ) )
		...
		$update_skin = array( 'fonts' => $fonts, 'style_variations' =>  $name, 'cufon_gradients' => $cufon_gradients_write, 'cufon_gradients_fonts' => $cufon_gradients_fonts );

	...
	function tooltip( $atts = null, $content = null ) {
		...
		// If ID is set then we echo out the custom trigger
			...
			jQuery(document).ready(function() {
				...
				jQuery('#{$custom_id}').cluetip({ attribute: 'rel', local:true, {$options} hideLocal: false, cursor: 'pointer', showTitle: false, waitImage: false, clickThrough: false, dropShadow: false, waitImage :false, onShow: function(e) { {$style}{$close}  }, fx: { open: 'fadeIn', openSpeed: 'fast' } });
			});
